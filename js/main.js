...
function handleCalculateClick(event) {
	...
	const pullsUntil = calculator.calculatePullsUntilPity(pityGoals, state.userInput.currentPity)
	const totalPasses = calculator.calculateTotalPasses(state.userInput.jades, state.userInput.specialPasses)
...
function handleBonusToggle(event) {
	const key = Object.keys(ui.DOMElements.bonusToggles).find((k) => ui.DOMElements.bonusToggles[k] === event.target)
	if (key) {
...
async function initialize() {
	...
	ui.DOMElements.toggleAllBonuses.addEventListener("change", handleToggleAllBonuses)
	Object.values(ui.DOMElements.bonusToggles).forEach((checkbox) => {
		checkbox.addEventListener("change", handleBonusToggle)
	})

	// Region change listener
	ui.DOMElements.regionSelector.addEventListener("change", (e) => updateRegion(e.target.value))
}

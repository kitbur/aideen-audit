...
export function getPityGoals(isGuaranteed) {
    const hardPity = isGuaranteed ? PITY_THRESHOLDS.HARD_GUARANTEED : PITY_THRESHOLDS.HARD_50_50;
    const softPity = isGuaranteed ? PITY_THRESHOLDS.SOFT_GUARANTEED : PITY_THRESHOLDS.SOFT_50_50;
...
export function calculateAmountNeeded(pullsUntil, totalPasses) {
    const neededPassesHardPity = Math.max(0, pullsUntil.pullsUntilHardPity - totalPasses);
    const neededPassesSoftPity = Math.max(0, pullsUntil.pullsUntilSoftPity - totalPasses);
...
function calculatePurchasePlan(neededJades, bonusToggles, regionData) {
    if (neededJades <= 0) {
        return { costString: `<span><span class="minorText">${regionData.currency}</span>0.00</span>`, packsString: 'None' };
    }
    ...
    const costString = `<span><span class="minorText">${regionData.currency}</span>${totalCost.toFixed(2)}</span>`;
...
export function calculateCost(amountNeeded, bonusToggles, regionData) {
    const hardPityPlan = calculatePurchasePlan(amountNeeded.neededJadesHardPity, bonusToggles, regionData);
    const softPityPlan = calculatePurchasePlan(amountNeeded.neededJadesSoftPity, bonusToggles, regionData);
